    ...
    for ix in range(R.shape[0]):
        ...
        for bbox_num in range(len(bboxes)):
            ...
            if curr_iou > best_iou:
                ...
                best_bbox = bbox_num
        # print(best_iou)
        if best_iou < config.class_min_overlap:
            continue
        else:
            w = x2 - x1
            h = y2 - y1
            ...
            if config.class_min_overlap <= best_iou < config.class_max_overlap:
                label = -1
            elif config.class_max_overlap <= best_iou:
        ...
        if label != -1:
            label_pos = 4 * label
            sx, sy, sw, sh = config.class_regr_std

...
from nets.frcnn_training import get_new_image_size
...
class Frcnn(object):
    _defaults = {'model_path':'model_data/voc_weights.h5',
    ...
    def _get_class(self):
        """
            这里get_class_name方法和YOlo中一样
        :return:
        """
        class_path = os.path.expanduser(self._defaults['classes_path'])
    ...
    def generate(self):
        ...
        self.num_class = len(self.class_name) + 1
        #获取rpn网络和classifier网络
        self.model_rpn, self.model_classifier = frcnn.get_predict_model(self.config,self.num_class)
        self.model_rpn.load_weights("/Users/aragaki/artificial/pythonProject/week14/logs/epoch007-loss2.050-rpn0.655-roi1.395.h5",by_name=True)
        self.model_classifier.load_weights(self._defaults['model_path'],by_name=True,skip_mismatch=True)
        print('{} model,anchors, and classes loaded'.format(model_path))

        hsv_tuple = [(x / len(self.class_name),1,1) for x in range(len(self.class_name))]
        self.colors = list(map(lambda x: colorsys.hsv_to_rgb(*x),hsv_tuple))
        self.colors = list(map(lambda x:(int(x[0]*255),int(x[1]*255),int(x[2]*255)),self.colors))

    def get_image_out_length(shpae,old_width,old_height):
        '''
          这里是根据公式计算卷积输出结果
        :param old_width:
        :param old_height:
        :return:
        '''
        def _get_out_length(input_length):
            filter_size = [7,3,1,1]
            padding = [3,1,0,0]
            strides = 2
            for i in range(4):
                input_length = (input_length+2*padding[i]-filter_size[i]) //strides + 1
            return input_length
        return _get_out_length(old_width), _get_out_length(old_height)
    def detect_image(self,image):
        image_shape = np.array(np.shape(image)[0:2])
        old_width = image_shape[1]
        old_height = image_shape[0]
        old_image = copy.deepcopy(image)
        width, height = get_new_image_size(old_width,old_height)
        ...
        anchors = get_anchors(self.get_image_out_length(width,height),width,height)
        ...
        for jk in range(R.shape[0]//self.config.num_rois + 1):
            ...
            for ii in range(P_cls.shape[1]):
                ...
                tx /= self.config.class_regr_std[0]
                ty /= self.config.class_regr_std[1]
                tw /= self.config.class_regr_std[2]
                th /= self.config.class_regr_std[3]
        ...
        results = np.array(
            self.bbox_util.nms_for_out(np.array(labels), np.array(probs), np.array(boxes), self.num_class - 1, 0.4))
